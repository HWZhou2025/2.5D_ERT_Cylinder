%编者信息：周红伟，最后更新日期20251005
%此程序用于进行模型正演计算
clear
clc
tic;
IK=[-500 0 0.02;500 0 -0.02];%设置供电电极位置及大小信息，[Xa Ya Ia;Xb Yb Ib]，单位为毫米和安培
R=500;%树的半径,单位为毫米
gd=[IK(1,1) IK(1,2);IK(2,1) IK(2,2)];
%%开始设置模型
fd=@(p) p(:,1).^2+p(:,2).^2-R^2;%定义模型的边界，在内小宇0，在外大于0
fh=@(p) gaolan_xishu(p,IK);%定义网格稀疏程度的函数
[p,t]=distmesh2d(fd,fh,4,[-500,-500;500,500],gd);%网格化函数，distmesh2d（模型边界，稀疏程度函数，稀疏程度比例，[xmin,ymin;xmax,ymax],固定的节点坐标信息[1 2;2 0;2 9]可固定三个节点）
% % %网格化xishu
% load('p8135');load('t8135');
[m,n]=size(t);
[m1,n1]=size(p);
rho=ones(m,1)*100;
figure
