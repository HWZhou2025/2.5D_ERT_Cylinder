...
% fh=@(p) xishu_model2(p);%定义网格稀疏程度的函数
% [p,t]=distmesh2d(fd,fh,1.5,[-200,-200;200,200],[gd]);%网格化函数，distmesh2d（模型边界，稀疏程度函数，稀疏程度比例，[xmin,ymin;xmax,ymax],固定的节点坐标信息[1 2;2 0;2 9]可固定三个节点）
...
figure
for i=1:1:m
    rm1=(p(t(i,1),1)-70)^2+(p(t(i,1),2)-70)^2-4900;rm2=(p(t(i,2),1)-70)^2+(p(t(i,2),2)-70)^2-4900;rm3=(p(t(i,3),1)-70)^2+(p(t(i,3),2)-70)^2-4900;
