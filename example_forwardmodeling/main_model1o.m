...
% fh=@(p) xishu_model1(p);%定义网格稀疏程度的函数
% [p,t]=distmesh2d(fd,fh,2,[-200,-200;200,200],[gd]);%网格化函数，distmesh2d（模型边界，稀疏程度函数，稀疏程度比例，[xmin,ymin;xmax,ymax],固定的节点坐标信息[1 2;2 0;2 9]可固定三个节点）
[m,n]=size(t);
[m1,n1]=size(p);
rho=ones(m,1)*100;
for i=1:1:32
...
figure
for i=1:1:m
    rm1=(p(t(i,1),1)-0)^2+(p(t(i,1),2)-0)^2-10000;rm2=(p(t(i,2),1)-0)^2+(p(t(i,2),2)-0)^2-10000;rm3=(p(t(i,3),1)-0)^2+(p(t(i,3),2)-0)^2-10000;
...
b=1;umn=zeros(32*15,4);
for i=1:1:15
    tic
    for j=1:1:32
tic
            IK=[gd(j,1) gd(j,2) 0.2;gd(j+1,1) gd(j+1,1) -0.2];
            kg1=kg(1:6,:);  
            for ii=1:1:6
                K=gaolan_K(rho,p,t,kg1(ii,1));
                I=zeros(m1,1);
                I(ngd(j,1),1)=IK(1,3);I(ngd(j+1,1),1)=IK(2,3);
                V(:,ii)=K^(-1)*I;
            end
            umn(b,3)=V(ngd(j+i+1,1),:)*kg1(:,2);umn(b,4)=V(ngd(j+2+i,1),:)*kg1(:,2);umn(b,1)=i;umn(b,2)=j;
            b=b+1;
...
xlswrite('model1o20260303.xlsx',umn);
